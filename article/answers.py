...
def message(request):
    ...
    if is_first_use:
        ...
        if content == setting_list[0]:
            ...
            prev_select[user_key] = '저장한 뉴스 초기화'
            return JsonResponse({'message': {'text': '저장한 뉴스를 초기화 하시겠습니까?'},
                                 'keyboard': {'type': 'buttons',
                                              'buttons': ['Y', 'N']}
                                 })

    elif prev_select.get(user_key) == u'저장한 뉴스 초기화':
        reset_globals(user_key)
        if content == 'Y':
            ...
            return JsonResponse({'message': {'text': '저장한 뉴스를 초기화 하지 않았습니다.'},
                                 'keyboard': {'type': 'buttons',
                                              'buttons': first_button_list}
                                 })
